...
uint8_t screen_dot_red[32][8];    //ÏÔ´æ£¬±£´æµ±Ç°ÏÔÊ¾µÄÒ»ÆÁµãÕóÊı¾İ
uint8_t screen_dot_green[32][8];   //ÏÔ´æ£¬±£´æµ±Ç°ÏÔÊ¾µÄÒ»ÆÁÂÌÉ«µãÕóÊı¾İ
...
static void LED_Display (void const *arg) {
	...
	bool fresh_screen_dot[MAX_AREA_NUMBER];				 //ÖØĞÂÌî³äÏÔ´æ
	...
	while(1){
		...
		for( area_no=0;area_no<screen.area_number;area_no++ ){
			...
			for( row=area[area_no].y; row<(area[area_no].y+area[area_no].height);row++){
				/////////////////////////////////////
				//if(row-area[area_no].y>=16 ) continue;  //ÔİÊ±Ê¹ÓÃ
				...
				while( col<area[area_no].length ){
					if( *pStr <= 126){      //Ó¢ÎÄ×Ö·û
						current_row_dot[col++] = ascii_Dot[ *pStr - ' ' ][row - area[area_no].y ] ;    //»ñÈ¡µãÕó
						pStr++ ;
					}
					else{    //ºº×Ö×Ö·û
		...
		speed[area_no]--;
		if( speed[area_no] < 1 ){
			fresh_screen_dot[area_no] = true;    //ÖØĞÂÌî³äÏÔ´æ
